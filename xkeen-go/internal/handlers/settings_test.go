...
func TestGetXraySettings_DefaultsWhenNoConfigFile(t *testing.T) {
	...
	if result["access_log"] != filepath.FromSlash("/opt/var/log/xray/access.log") {
		t.Errorf("expected default access_log path, got %v", result["access_log"])
	}
	if result["error_log"] != filepath.FromSlash("/opt/var/log/xray/error.log") {
...
func TestUpdateLogLevel_CreatesFileIfNotExists(t *testing.T) {
	...
	if logCfg.Log.Access != filepath.FromSlash("/opt/var/log/xray/access.log") {
		t.Errorf("expected default access path, got %q", logCfg.Log.Access)
	}
	if logCfg.Log.Error != filepath.FromSlash("/opt/var/log/xray/error.log") {
