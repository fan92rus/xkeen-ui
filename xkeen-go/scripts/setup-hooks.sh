...
echo "Installing pre-commit + pre-merge-commit hooks..."
mkdir -p "$HOOKS_DIR"
cp "$REPO_ROOT/xkeen-go/scripts/pre-commit" "$HOOKS_DIR/pre-commit"
chmod +x "$HOOKS_DIR/pre-commit"
cp "$REPO_ROOT/xkeen-go/scripts/pre-merge-commit" "$HOOKS_DIR/pre-merge-commit"
chmod +x "$HOOKS_DIR/pre-merge-commit"

echo "✅ Hooks installed!"
echo ""
echo "The hooks will run:"
...
echo "  - changelog check on master commits/merges (CI also enforces on PR)"
echo ""
echo "Bypass with: git commit --no-verify / git merge --no-verify"
